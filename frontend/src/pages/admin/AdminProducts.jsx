    ...
    try {
      const payload = {
        ...
        weight_options: (form.weight_options || []).map(opt => ({
          ...
          wholesalePrice: opt.wholesalePrice ? Number(opt.wholesalePrice) : null,
        ...
        <FormShell title={editing === "new" ? "Add Product" : "Edit Product"} onClose={close} onSubmit={save} submitting={submitting}>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button type="button" onClick={() => setForm({ ...form, weight_options: [...(form.weight_options || []), { weight: "", unit: "g", price: 0, sale_price: "", retailerPrice: "", wholesalePrice: "", normalPrice: "", stock: 0, image: "" }] })} className="text-xs bg-[#0f4d2e]/10 text-[#0f4d2e] px-3 py-1.5 rounded-full font-semibold hover:bg-[#0f4d2e]/20 transition-colors">
                + Add Option
              </button>
            </div>
                ...
                {form.weight_options.map((opt, idx) => (
                  <div key={idx} className="flex flex-col gap-3 p-4 bg-[#fdfbf7] border border-[#6b3e1f]/10 rounded-xl relative group">
                    ...
                    <div className="flex flex-wrap items-end gap-3">
                      ...
                      <label className="flex-1 min-w-[80px]">
                        <span className="text-[10px] font-bold uppercase tracking-wider text-[#6b3e1f] mb-1 block">Wholesale (₹)</span>
                        <TextInput type="number" min="0" value={opt.wholesalePrice ?? ""} onChange={(e) => { const newOpts = [...form.weight_options]; newOpts[idx].wholesalePrice = e.target.value; setForm({ ...form, weight_options: newOpts }); }} />
                      </label>
                      <label className="flex-1 min-w-[80px]">
